...
/*          .style('width', '100%')
    ...
    function drawRiver() {
      gMap
        ...
        .attr('stroke', function(d){
          return (d.color != undefined ? d.color : '#CCECF4'); //Modified by Sebin: if defined apply that color else use the default
        })
        .attr('fill', 'none')
    ...
    function drawInterchanges() {
      var fgColor = '#000000';
      var bgColor = '#ffffff';
      var notStarted = '#37474f';
      var onSchedule = '#00ff55';
      var behindLate = '#ffe900';
      var critical = '#ff0055';
      ...
      gMap
        ...
        .append('g')
        .attr('id', function(d) {
          return (((d.name).replace('(','_')).replace('/','_')).replace(')','');
        })
        .on('click', function() {
          ...
          if(d.progress != undefined){
              if(d.progress == 1){
                return notStarted;
                  ...
                  blink('#'+(((d.name).replace('(','_')).replace('/','_')).replace(')',''), -1, 500);
                  $('#'+(((d.name).replace('(','_')).replace('/','_')).replace(')','')).toggle('pulsate').css({'fill':'#ffffff','stroke':'#ff0055'});
                  return critical;
              }else{
                  return bgColor;
              }
    ...
    function extractStations(data) {
      data.lines.forEach(function(line$$1) {
        for (var node = 0; node < line$$1.nodes.length; node++) {
          ...
          /*ADDED BY SEBIN*/
          /* START*/
            station$$1.labelColor = d.labelColor;
            station$$1.labelSize = d.labelSize;
            station$$1.progress = d.status;
          /*END*/
    ...
    function textPos(data) {
      ...
      switch (data.labelPos.toLowerCase()) {
        ...
        case 'se':
          ...
          break;
        case 's':
          pos = [0, -2 * lineWidthMultiplier * offset];
          textAnchor = 'middle';
          break;
        case 'sw':
          pos = [-offset / sqrt2, -1.4 * offset / sqrt2];
