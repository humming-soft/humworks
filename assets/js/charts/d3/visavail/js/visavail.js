  ...
  function chart(selection) {
    selection.each(function drawGraph(dataset) {
      ...
      var xScale = d3.time.scale()
          .domain([new Date("2016-12-01"), new Date("2019-12-31")])
          /*.domain([startDate, endDate])*/
          /*.domain(d3.extent(dataset, function(d) { var datesArray = []; datesArray.push(d.disp_data[0][0]); datesArray.push(d.disp_data[0][2]); return datesArray; }))*/
          .range([0, width])
          .clamp(1);
      ...
      var xAxis = d3.svg.axis()
          ...
          .tickFormat( function ( x ) {
              if (d3.time.year(x) < x) {
                  return moment(parseDate(x)).format('MMM').toUpperCase();
              } else {
                  return moment(parseDate(x)).format('YYYY');
              }
          });
      ...
      if (noOfDatasets) {
        svg.select('#g_axis').selectAll('line.vert_grid').data(xScale.ticks(d3.time.months))
            .enter()
            .append('line')
            .attr({
              'class': 'vert_grid',
...
/*        var tooltip = d3.select("body")
            ...
            div.html(function () {
              ...
              if (isDateOnlyFormat) {
                if (d[2] > d3.time.second.offset(d[0], 86400)) {
                  return output + moment(parseDate(d[0])).format('DD-MMM-YYYY')
                      + ' to ' + moment(parseDate(d[2])).format('DD-MMM-YYYY');
                }
      ...
      var xTicks = xScale.ticks(d3.time.months);
      ...
      if (!customCategories) {
        ...
        legend.append('text')
          ...
          legend.append('rect')
              ...
              .attr('height', 15)
              .attr('width', 15)
              .attr('class', 'piling_data')
              .on('click',function(){
                  d3.select(this).classed("piling_data", !d3.select(this).classed("piling_data"))
              });
          ...
          // legend -- restless
          legend.append('rect')
              .attr('x', width + margin.right - 510)
              .attr('y', paddingTopHeading)
              .attr('height', 15)
              .attr('width', 15)
              .attr('class', 'pilecaps_data');

          legend.append('text')
              .attr('x', width + margin.right - 510 + 20)
              .attr('y', paddingTopHeading + 8.5)
              .text('Pilecaps')
              .attr('class', 'legend');
          ...
          // legend -- light
          legend.append('rect')
              .attr('x', width + margin.right - 435)
              .attr('y', paddingTopHeading)
              .attr('height', 15)
              .attr('width', 15)
              .attr('class', 'piers_data'); //what is this?

          legend.append('text')
              .attr('x', width + margin.right - 435 + 20)
              .attr('y', paddingTopHeading + 8.5)
              .text('Piers')
              .attr('class', 'legend');
          ...
          // legend -- normal
          legend.append('rect')
              .attr('x', width + margin.right - 380)
              .attr('y', paddingTopHeading)
              .attr('height', 15)
              .attr('width', 15)
              .attr('class', 'pierhead_data'); //what is this?

          legend.append('text')
              .attr('x', width + margin.right - 380 + 20)
              .attr('y', paddingTopHeading + 8.5)
              .text('Pierhead')
              .attr('class', 'legend');
          ...
          legend.append('rect')
              ...
              .attr('height', 15)
              .attr('width', 15)
              .attr('class', 'bearing_data'); //what is this?
          ...
          legend.append('rect')
              ...
              .attr('height', 15)
              .attr('width', 15)
              .attr('class', 'deck_launching_data'); //what is this?

          legend.append('text')
              .attr('x', width + margin.right - 230 + 20)
              .attr('y', paddingTopHeading + 8.5)
              .text('Deck Launching')
              .attr('class', 'legend');

          legend.append('rect')
              .attr('x', width + margin.right - 115)
              .attr('y', paddingTopHeading)
              .attr('height', 15)
              .attr('width', 15)
              .attr('class', 'parapet_data'); //what is this?

          legend.append('text')
              .attr('x', width + margin.right - 115 + 20)
              .attr('y', paddingTopHeading + 8.5)
              .text('Parapet')
              .attr('class', 'legend');
      }
